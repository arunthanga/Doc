#!/usr/bin/env python3
"""Build a watermarked PDF of the Coconut Cider Vinegar DPR from the Markdown source.

- Converts DPR-Coconut-Cider-Vinegar.md to HTML (tables, fenced code).
- Applies a repeating "Arun" watermark on every page.
- Renders to DPR-Coconut-Cider-Vinegar.pdf via headless Chrome.
"""
import base64
import pathlib
import subprocess
...
MD = ROOT / "DPR-Coconut-Cider-Vinegar.md"
HTML = ROOT / "vinegar-dpr.html"
PDF = ROOT / "DPR-Coconut-Cider-Vinegar.pdf"
WATERMARK_TEXT = "Arun"
...
for chrome in ("google-chrome", "chromium", "chromium-browser"):
    if subprocess.run(["which", chrome], capture_output=True).returncode == 0:
        with tempfile.TemporaryDirectory(prefix="chrome-vinegar-dpr-") as profile:
            if PDF.exists():
                PDF.unlink()
